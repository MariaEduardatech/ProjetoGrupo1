...
function cadastrarUsuario(req, res) {
    ...
    // Passe os valores como parâmetro e vá para o arquivo usuarioModel.js
    usuarioModel.cadastrarUsuario(codigo, NomeUsuario, EmailUsuario, senha, cargo)
        .then(
            function (resultado) {
                res.json(resultado);
            }
...
function BuscarDados(req, res) {
    ...
    usuarioModel.BuscarDados(idEmpresa).then(function (resultado) {
        if (resultado.length > 0) {
            res.status(200).json(resultado); 
        } else {
...
function BuscarSensor(req, res) {
    ...
    var idEmpresa = req.params.idDaEmpresa;
    var statusSensor = req.params.statusSensor;

    usuarioModel.BuscarSensor(idEmpresa, statusSensor).then(function (resultado) {
        if (resultado.length > 0) {
            res.status(200).json(resultado); 
        } else {
...
function BuscarAlertaSensor(req, res) {

    var idEmpresa = req.params.idDaEmpresa;
    var talhao = req.params.talhao;

    usuarioModel.BuscarAlertaSensor(idEmpresa, talhao).then(function (resultado) {
        if (resultado.length > 0) {
            res.status(200).json(resultado); 
        } else {
            res.status(204).send("Nenhum dado do sensor encontrado!")
...
function BuscarQuantidadeSensor(req, res) {

    var idEmpresa = req.params.idDaEmpresa;
    var talhao = req.params.talhao;

    usuarioModel.BuscarQuantidadeSensor(idEmpresa, talhao).then(function (resultado) {
        if (resultado.length > 0) {
            res.status(200).json(resultado); 
        } else {
            res.status(204).send("Nenhum sensor encontrado!")
